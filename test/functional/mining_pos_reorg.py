...
class ReorgStakeTest(SupernodeCoinTestFramework):
    ...
    def check_money_supply(self, expected_bc, expected_zsuno):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected SUNO and zSUNO supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bc))
            for denom in node['zSUNOsupply']:
                assert_equal(node['zSUNOsupply'][denom], DecimalAmt(expected_zsuno[denom]))
    ...
    def run_test(self):
        ...
        # Check SUNO and zSUNO supply at the beginning
        # ------------------------------------------
        # zSUNO supply: 2 coins for each denomination
        expected_zsuno_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # SUNO supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zsuno_supply)
        ...
        # Verify that SUNO and zSUNO supplies were properly updated after the spends and reorgs
        self.log.info("Check SUNO and zSUNO supply...")
        ...
        expected_zsuno_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zsuno_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zsuno_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zsuno_supply)
