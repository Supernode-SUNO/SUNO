...
const char * const SupernodeCoin_CONF_FILENAME = "supernodecoin.conf";
const char * const SupernodeCoin_PID_FILENAME = "supernodecoin.pid";
const char * const SupernodeCoin_MASTERNODE_CONF_FILENAME = "masternode.conf";


// SupernodeCoin only features
// Masternode
...
#else
    const char* pszModule = "supernodecoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\SupernodeCoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\SupernodeCoin
// Mac: ~/Library/Application Support/SupernodeCoin
// Unix: ~/.supernodecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SupernodeCoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SupernodeCoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SupernodeCoinParams
    // Mac: ~/Library/Application Support/SupernodeCoinParams
    // Unix: ~/.supernodecoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SupernodeCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "SupernodeCoinParams";
#else
    // Unix
    return pathRet / ".supernodecoin-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", SupernodeCoin_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", SupernodeCoin_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty supernodecoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override supernodecoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", SupernodeCoin_PID_FILENAME));
