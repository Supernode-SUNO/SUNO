...
bool CSunoStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CSunoStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CSunoStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CSunoStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CSunoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CSunoStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CSunoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CSunoStake::GetUniqueness() const
{
    //The unique identifier for a SUNO stake is the outpoint
...
CBlockIndex* CSunoStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CSunoStake::ContextCheck(int nHeight, uint32_t nTime)
{
