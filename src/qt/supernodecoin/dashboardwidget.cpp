...
#include "qt/supernodecoin/dashboardwidget.h"
#include "qt/supernodecoin/forms/ui_dashboardwidget.h"
#include "qt/supernodecoin/sendconfirmdialog.h"
#include "qt/supernodecoin/txrow.h"
...
DashboardWidget::DashboardWidget(SupernodeCoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	setCssProperty(ui->labelSquareSuno, "square-chart-suno");
	setCssProperty(ui->labelSuno, "text-chart-suno");
	...
	setCssProperty(ui->labelAmountSuno, "text-stake-suno-disable");
	...
	if (window)
		connect(window, &SupernodeCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
	if (walletModel && walletModel->getOptionsModel()) {
		...
		connect(ui->pushImgEmpty, &QPushButton::clicked, window, &SupernodeCoinGUI::openFAQ);
		connect(ui->btnHowTo, &QPushButton::clicked, window, &SupernodeCoinGUI::openFAQ);
	...
	// update the display unit, to not use the default ("SUNO")
	updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	chartData->amountsByCache = getAmountBy(); // pair SUNO
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			suno = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalSuno += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesSuno.append(suno);

		int max = std::max(suno, suno);
...
void DashboardWidget::onChartRefreshed()
{
	...
	set0->append(chartData->valuesSuno);

	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalSuno > 0) {
		setCssProperty(ui->labelAmountSuno, "text-stake-suno");
	}
	else {
		setCssProperty(ui->labelAmountSuno, "text-stake-suno-disable");
	}
	forceUpdateStyle({ ui->labelAmountSuno });
	ui->labelAmountSuno->setText(GUIUtil::formatBalance(chartData->totalSuno, nDisplayUnit));
