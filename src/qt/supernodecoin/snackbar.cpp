...
#include "qt/supernodecoin/forms/ui_snackbar.h"
#include "qt/supernodecoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SupernodeCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SupernodeCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
