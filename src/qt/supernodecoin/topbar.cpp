...
#include "qt/supernodecoin/topbar.h"
#include "qt/supernodecoin/forms/ui_topbar.h"
#include "qt/supernodecoin/lockunlock.h"
#include "qt/supernodecoin/qtutils.h"
#include "qt/supernodecoin/receivedialog.h"
#include "qt/supernodecoin/loadingdialog.h"
...
TopBar::TopBar(SupernodeCoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopSuno}, "amount-small-topbar");
    setCssProperty({ui->labelAmountSuno}, "amount-topbar");
    setCssProperty({ui->labelPendingSuno, ui->labelImmatureSuno}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("Supernode Coin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // SUNO Total
    QString totalSuno = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);

    // SUNO
    // Top
    ui->labelAmountTopSuno->setText(totalSuno);
    // Expanded
    ui->labelAmountSuno->setText(totalSuno);
    ui->labelPendingSuno->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureSuno->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
