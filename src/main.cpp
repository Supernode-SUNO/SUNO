...
#if defined(NDEBUG)
#error "SupernodeCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usuno) are considered zero fee (for relaying, mining and transaction creation)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 7000 && isPoSActive) {
		nSubsidy = 3.00 * COIN;
	}
	else if (nHeight <= 14000 && nHeight > 7000) {
		nSubsidy = 6.00 * COIN;
	}
	else if (nHeight <= 3273733 && nHeight > 14000) {
		nSubsidy = 9.00 * COIN;
	}
	else if (nHeight == 3273733) {
		nSubsidy = 3.00 * COIN;
	}
	else {
		nSubsidy = 0;
...
{
    util::ThreadRename("Supernodecoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> sunoInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                sunoInputs.push_back(stakeIn);
        }
        const bool hasSUNOInputs = !sunoInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasSUNOInputs) {
                    ...
                    for (const CTxIn& sunoIn : sunoInputs)
                        if(sunoIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasSUNOInputs) {
                            for (const CTxIn& stakeIn : sunoInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // SupernodeCoin: We use certain sporks during IBD, so check to see if they are
