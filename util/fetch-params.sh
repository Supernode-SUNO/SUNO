...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/SupernodeCoinParams"
    else
        PARAMS_DIR="$HOME/.supernodecoin-params"
    fi
fi
...
Failed to fetch the SupernodeCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
SupernodeCoin - fetch-params.sh

This script will fetch the SupernodeCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common SupernodeCoin zkSNARK parameters. Note that it is
